...
- **Open sub-question (does not block the controller):** whether the 2d ~10×/10× `AddForce` deficits are a project physics-substep/units setting or inherent `bAccelChange` behavior. Worth a quick check when the build resumes, but the integrator path sidesteps it.
