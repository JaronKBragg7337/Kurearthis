...
Assume only what exists in this repository and the verified live scene.

The current objective is proving a real planetary world model. Additional gameplay, narrative, economic, social, ownership, vehicle, settlement, or progression systems must be explicitly introduced rather than inferred.
...
## Asset law
Every asset is either:
- `IN-HOUSE` — built from scratch in this project
- `OPEN-SOURCE` — sourced externally

Open-source assets must keep their source URL and license noted in `ASSETS_MANIFEST.md`.

## Restart law
If the live project state and the log disagree, trust the live project first. Record the divergence. Fix the state before adding any new work. Never paper over a mismatch with assumptions.

## Crash law
Any session that hit a usage limit or crashed mid-task must be treated as a potential contamination event. Verify the live scene before continuing. Do not assume the log is complete.