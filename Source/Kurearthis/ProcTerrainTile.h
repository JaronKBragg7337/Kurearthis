...
public:
	...
	/** Resolution for tiles inside the real-DEM region (~128 → ~39 m/quad, captures ~100 m
	 *  rivers). Viable once collision cooks async + tiles generate ≤1/tick (L1). */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ProcTerrain")
	int32 DemResolution = 128;
